...
import {findLocalizedStringByPredicate, findStringByPredicate, findStringsByPredicate, stringToNTriples, Triple} from 'ntriples-collection'
...
function parseRelease(triples: ReadonlyArray<Triple>, identifier: string): Release {
  const releaseTriples = triples.filter(triple => triple.subject === identifier)
  const findString = (predicate: string) => nullToEmpty(findStringByPredicate(releaseTriples, predicate, ''))
  const findStrings = (predicate: string) => findStringsByPredicate(releaseTriples, predicate)
  ...
  const release: Release = { audiences, identifier, created, revision, feature, language, dataDump ,subjects, uriSpace}
  return release; 
}

function parseMaintainer(triples: ReadonlyArray<Triple>, identifier: string, access: ReadonlyArray<string>): AgentAccess {
  const maintainerTriples = triples.filter(triple => triple.subject === identifier)
  const findString = (predicate: string) => nullToEmpty(findStringByPredicate(maintainerTriples, predicate, ''))
  const findStrings = (predicate: string) => findStringsByPredicate(maintainerTriples, predicate)
...
export default function parseMutuality(language: string, content: string): Mutuality {
  ...
  const findLocalized = (predicate: string) => nullToEmpty(findLocalizedStringByPredicate(mainTriples, predicate, language, [], ''))
  const findString = (predicate: string) => nullToEmpty(findStringByPredicate(mainTriples, predicate, ''))
  const findStrings = (predicate: string) => findStringsByPredicate(mainTriples, predicate)
  ...
  const asMaintainer = (predicate: string) => parseMaintainer(triples, predicate, writeAccess)
